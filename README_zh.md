...
- **ActuCore（L2）** —— 同一层的执行模型侧：VLA 策略、导航、抓取、运动控制、全身控制。它们以 `processor` 类型的 MCP 设备接入，所以任何「输入目标、输出运动指令」的模型都能用同一套方式挂上来。ActuCore 是**架构里的一个位置**，不是本仓库附带的组件 —— 具体用哪些模型按机器人选型决定。
