...
回复入站 Bot 消息时，`mention_open_id` 必须与当前触发事件的 `source_message_id` 一起使用，而且只能 `@` 原发送机器人。主动发起时使用 `trusted_bot_id`，不直接填写目标群或 `open_id`。机器人不能选用历史消息 ID、机器人单聊或发送附件。其他机器人没有上述标记时，明确 `@` 当前机器人的消息按一次“需要回复”的请求处理。
