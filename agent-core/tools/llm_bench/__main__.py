...
用法见 README.md。典型：
    ...
    python3 /work/tools/llm_bench --config bench.yaml --count 50 --repeats 2
...
def parse_args(argv=None):
    ...
    ap.add_argument('--sampling', choices=['even', 'recent', 'random'],
                    help='抽样方式（覆盖配置）')
    ap.add_argument('--repeats', type=int, help='计分轮数（覆盖配置）')
    ap.add_argument('--warmup', type=int, help='预热轮数（覆盖配置）')
...
def _run(args, cfg, groups, transport, resume_dir, meta) -> int:
    ...
    # ── 正式跑 ────────────────────────────────────────────────────────────
    total = (cfg['run']['warmup'] + cfg['run']['repeats']) * len(payloads) * len(groups)
    ...
    expected = (cfg['run']['warmup'] + cfg['run']['repeats']) * len(payloads) * len(groups)
