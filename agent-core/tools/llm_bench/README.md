...
**总请求数 = `count × 组数 × 2`**（冷轮 + 热轮），启动时会打印出来，看一眼再决定要不要跑。

输出在 `resource/llm_bench/<时间戳>/`，宿主机上是 `/opt/phanthy-motus/data/llm_bench/<时间戳>/`，可以直接 `cat`，不用进容器：

| 文件 | 内容 |
|---|---|
| `report.md` | 人读报告：结论 → 总表 → 配对比较 → 冷轮 vs 热轮 → 失败分析 → 缓存 → 方法与局限 |
...
### 冷轮 + 热轮，两遍都算数（`run.warmup`）

跑两遍：**冷轮**（每条 payload 第一次被请求）+ **热轮**（同一批再走一遍）。

冷轮**不是丢掉的预热**。它是唯一一次每条 payload 都没被自己热过的完整测量，所以：

- **冷态延迟**只能从它读。机器人经常长时间空闲后被唤醒，那一次的体验就是冷态。
- **真实缓存率**也只能从它读。热轮里每条 payload 都是原样重放，命中必然接近 100%，那量的是「同一个请求重发」，不是生产的前缀增长。

热轮的用处是**组间公平对比**：所有组都在同样热的状态下比，消除谁先跑谁吃冷缓存的不对称。实测中一个入口第一轮缓存命中中位只有 48.8% 被判「慢 2.5 倍」，喂热后（98.5%）实际只慢 1.8 倍——不做热轮就是在测「谁的缓存先被我喂热」。

**两态结论不一致时报告会明确点出来。** 一个入口只有热了才快，对会长时间空闲的机器人是实质缺点。持续对话为主看热轮，零星唤醒为主看冷轮。

### 不做重复轮，显著性用统计检验

**刻意不提供「同一条 payload 重复 N 次」。** 重复重放量的是「同一个请求重发」——服务端前缀缓存必然命中，延迟分布和真实负载不是一回事。用一个不代表真实负载的测量去校准另一个测量，逻辑上站不住。
...
**缓存分冷热两列** —— 冷轮命中是接近生产的口径；热轮命中必然接近 100%（原样重放，命中来自自己），只适合组间横向比，**不能当生产缓存率读**。
...
| | 用途 |
|---|---|
| `even`（默认） | 跨越整个文件均匀取，覆盖各种 prompt 规模 —— **比延迟**用这个 |
| `trace` | 按整条 trace 取并保留轮次顺序 —— **比缓存**用这个 |
| `recent` / `random` | 备选 |

只取开头会让样本集中在某一段会话上，prompt 长度和缓存形态都不具代表性，所以默认是 `even`。

但 **`even` 测不出真实缓存率**。生产里 agent 是一轮接一轮，每轮 prompt 在上一轮基础上追加，所以同一条 trace 内相邻请求的消息前缀重叠很高；`even` 把这个连续性打散了。实测于 orin6（133 条记录 / 51 条 trace）：
...
`trace` 模式按整条 trace 取、保留 trace 内顺序，并优先多轮 trace（单条记录的 trace 不产生前缀增长，对缓存测量没贡献——实测语料里 51 条 trace 有 15 条是单条的），复现生产的前缀增长形态。报告会根据实际用的抽样方式，标注缓存数字可不可信。
