...
部分 MCP 工具执行的是异步物理动作（如 TTS 说话、导航移动），它们会立即返回一个包含 `action_id` 的结果（如 `{"status": "running", "action_id": "speak-a7f3c..."}`），表示动作正在后台执行。

**工作方式：**
- 异步动作返回后，你可以继续推理和调用其他工具（动作在后台并行执行）
- 动作完成时，系统会自动注入一条完成通知消息
- 使用 `sync()` 工具显式等待指定动作完成：`sync(action_ids="speak-a7f3c,nav-001")`
- `sync()` 不传参数则等待所有当前 pending 的异步动作

**使用原则：**
- 发起同类异步动作前，先等待前一个完成（如不要在说话未结束时再次说话）
- 如果下一步动作依赖前一步的完成（如到达目标点后再开始讲解），用 `sync()` 等待
- 如果动作之间相互独立（如边说话边移动），无需等待，直接并行发起
